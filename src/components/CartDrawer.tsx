...
import { X, ShoppingBag, Minus, Plus, Trash2, ArrowRight, Package, ExternalLink } from 'lucide-react';
import { useCart } from '../context/CartContext';
import { getShopifyConfigured } from '../lib/shopify';
...
export default function CartDrawer({ onCheckout }: Props) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 z-50 h-full w-full sm:w-[420px] bg-charcoal-950 border-l border-charcoal-800/60 flex flex-col transition-transform duration-300 ease-out ${
      ...
      >
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="px-5 py-5 border-t border-charcoal-800/50 space-y-4 bg-charcoal-900/40">
            {/* Subtotal */}
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-gold-gradient text-charcoal-950 font-bold text-sm tracking-widest uppercase py-4 rounded hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-70"
            >
              {checkoutLoading ? (
                <><div className="w-4 h-4 border-2 border-charcoal-950/30 border-t-charcoal-950 rounded-full animate-spin" /> Processing...</>
              ) : isShopifyConfigured ? (
